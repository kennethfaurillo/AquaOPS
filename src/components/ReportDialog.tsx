...
import { Averaging, Datalogger, ReportParameters } from "./Types"
...
type LinkType = { csv: HTMLAnchorElement | null, json: HTMLAnchorElement | null, xlsx: boolean | null }
const initialLinkState: LinkType = { csv: null, json: null, xlsx: null }
...
function ReportDialog({ reportDialogOpen, setReportDialogOpen, loggerInfo, allowedDates }: ReportDialogProps) {
    ...
    const [link, setLink] = useState<LinkType>(initialLinkState)
    const resetLinks = () => setLink(initialLinkState)
    // Which logs are checked 
    const [reportChecked, setReportChecked] = useState<ReportParameters>({
        param: null, // flow/pressure/voltage
        averaging: 'none',
        totalizerNet: false,
    ...
    const [allowedTotalizerDates, setAllowedTotalizerDates] = useState<string[]>([])
    const { user } = useAuth()

    const fetchTotalizerDates = async (loggerId: string) => {
        const logDatesResponse = await axios.get<string[]>(`${import.meta.env.VITE_API}/api/totalizer_log_dates/${loggerId}`, { withCredentials: true })
            ...
            <DialogContent>
                ...
                {loggerInfo ?
                    <div className="text-center">
                        <Tabs defaultValue="flow-pressure" onValueChange={() => {
                            ...
                            <TabsContent value="flow-pressure"  >
                                ...
                                <div className="mt-1">
                                    ...
                                    <ToggleGroup type="single" value={reportChecked.averaging ?? 'none'} onValueChange={(value: Averaging) => {
                                        if (!value) return
                                        setReportChecked({
                                            ...reportChecked,
                                            averaging: value
                            ...
                            <TabsContent value="totalizer">
                                ...
                                {loggerInfo.Type.includes("flow") ?
                                    <div className="grid">
                                        <span className="space-x-1 mx-4">
                                            <Checkbox id="cbTotalizerNet" checked={reportChecked.totalizerNet} onCheckedChange={(isChecked: boolean) => {
                                                setReportChecked({
                                                    ...reportChecked,
                                        ...
                                        <span className="space-x-1 mx-4">
                                            <Checkbox id="cbTotalizerPositive" checked={reportChecked.totalizerPositive} onCheckedChange={(isChecked: boolean) => {
                                                setReportChecked({
                                                    ...reportChecked,
                                        ...
                                        <span className="space-x-1 mx-4 overflow-hidden">
                                            <Checkbox id="cbTotalizerNegative" checked={reportChecked.totalizerNegative} onCheckedChange={(isChecked: boolean) => {
                                                setReportChecked({
                                                    ...reportChecked,
                        ...
                        <ToggleGroup type="single" value={reportFileType} onValueChange={(value: ReportFileType) => {
                            if (value) {
                ...
                <div className="grid grid-cols-2 gap-x-2">
                    {/* Enable button when a param is selected (flow/pressure/voltage or net/fwd/rev volume) and a time range is set */}
                    ...
                    {Object.values(link).some((value) => value !== null) && !loadingReport ? <Button className="bg-green-500" onClick={() => {
                        if (reportFileType == 'csv') {
                            if (!link.csv) {
                                toast.error("CSV link not available")
                                return
                            }
                            link.csv.click()
                        }
                            ...
                            if (!link.json) {
                                toast.error("JSON link not available")
                                return
                            }
                            link.json.click()
                        }
