...
import { parseLoggerName } from "./map/utils"
                ...
                <div className="grid grid-cols-2 gap-x-2">
                        ...
                        <Button className="bg-piwad-lightyellow-500 text-black" onClick={async () => {
                            ...
                            try {
                                ...
                                if (reportFileType == 'json') {
                                    ...
                                    const filename = `${parseLoggerName(loggerInfo.Name) + '_' + loggerInfo.LoggerId + '_' + moment(date?.from).format('YYYY-MM-DD')}`
                                    console.log(`${parseLoggerName(loggerInfo.Name) + '_' + loggerInfo.LoggerId + '_' + moment(date?.from).format('YYYY-MM-DD')}`)
                    ...
                    {link && !loadingReport ? <Button className="bg-green-500" onClick={() => {
                        if (reportFileType == 'csv') {
                            ...
                            XLSX.writeFile(workbook, `${parseLoggerName(loggerInfo.Name) + '_' + loggerInfo.LoggerId + '_' + moment(date?.from).format('YYYY-MM-DD')}.xlsx`)
                        }
